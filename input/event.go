...
import (
	...
	"github.com/julkipli/filebeat/harvester/reader"
	"github.com/julkipli/filebeat/input/file"
...
func (e *Event) ToMapStr() common.MapStr {
	...
	if e.JSONConfig != nil && len(jsonFields) > 0 {
		...
		//  if strings.HasSuffix(*e.Text,"Exception") {
		//  	event["cause"] = *e.Text
		//  }

		if strings.Contains(*e.Text,"Caused by:") {
			event["caused by"] = *e.Text
		} else if strings.Contains(*e.Text,"Exception") {
			event["exception"] = *e.Text
		} else if strings.Contains(*e.Text,"ERROR") {
				event["exception"] = *e.Text
		}
